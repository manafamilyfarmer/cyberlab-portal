...
Last updated: 2026-07-03 · HEAD: a10834e
...
- Wazuh forwarding part 1 (JSON emit) — every write_audit() ALSO emits one compact JSON line to AUDIT_LOG_PATH (/var/cyberlab-portal-logs/audit.jsonl, dedicated volume, NOT web-served, 0640 app:app), IN ADDITION to the unchanged AuditLog DB row. 'cyberlab.audit' logger + RotatingFileHandler (50MiB x5, size-capped) + JSONL formatter; stable schema (@timestamp/event_type/category/actor/actor_role/target_type/target_id/source_ip/result/detail/host); recursive secret-scrub. Emit is log-and-continue (a logging failure never breaks the audited action). Proven: valid JSONL across auth/admin/reaper/provisioning + ok/error/info; both sinks aligned (DB rows == JSONL lines); scrub redacts password/token/key/db_password/nested private_key; non-fatal + recovery; real worker-written provision.*/deprovision.ok in the shared file. Zero residue. **Part 2 (ship to VM109 manager) pending VM109 characterization.**
...
- Wazuh forwarding PART 2: ship AUDIT_LOG_PATH (audit.jsonl) to the VM109 manager — agent/syslog/API matched to VM109's config once known. Part 1 (JSON emit) DONE. Closes B2 observability.
